    ...
    with q_col3:
        ...
        if st.button("🚀 Book Now", key="q_book_btn", use_container_width=True):
            if q_time:
                ...
                if active_count >= 12:
                    st.error("Limit Reached (Max 12 active)")
                    add_log("Access Denied", f"{sub_community} Villa {villa} reached active booking limit (12)")
                elif daily_count >= 2:
...
with tab2:
    ...
    st.info(f"App allows 12 Active bookings spanning 14 days, A maximum of 2 active bookings per day. Current date choice timing: **{timing_msg}**")
    ...
    with col_status1: st.info(f"Total active bookings: **{active_count} / 12**")
    ...
    if st.button("Book This Slot", type="primary"):
        ...
        elif active_count_latest >= 12: 
            st.error("🚫 Overall limit reached. You cannot have more than 12 active bookings total.")
            add_log("Access Denied", f"{sub_community} Villa {villa} reached active booking limit (12)")
