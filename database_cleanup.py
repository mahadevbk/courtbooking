...
if not st.session_state.authenticated:
    ...
    st.subheader("🛡️ Resident Email Verification")
        st.caption("One-time verification code or registered account login.")
        ...
        if not st.session_state.otp_sent:
            ...
            if st.button("Continue / Send Code", type="primary", width='stretch'):
                ...
                if not otp_email_input or "@" not in otp_email_input:
                    ...
                    # --- DISCREET BACKEND CHECK FOR COACH ---
                    ...
                    if coach_check and coach_check.data:
                        # Instantly route the coach straight into their dashboard with zero friction
                        st.session_state.authenticated = True
...
if st.session_state.get('is_coach'):
    ...
    # Build a detailed list showing current utilization (active/limit) for each villa
    villa_display_list = []
    for v in assigned_villas:
        sub = v['sub_community']
        villa_num = v['villa']
        current_active = get_active_bookings_count(villa_num, sub)[cite: 1, 4]
        villa_limit = get_active_booking_limit(sub, villa_num)[cite: 1, 4] # Respects 8 for Legends of Mira, 6 for standard
